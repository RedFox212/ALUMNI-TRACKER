...
<header class="h-16 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border-b border-slate-100 dark:border-slate-800 px-8 flex items-center justify-between sticky top-0 z-30 transition-all duration-500">
    ...
    <div class="flex items-center gap-4 text-slate-400">
        <span class="text-sm font-medium text-slate-800 dark:text-white uppercase tracking-tighter"><?php echo $topbar_title ?? 'Portal'; ?></span>
        <span class="w-1 h-1 bg-slate-200 dark:bg-slate-700 rounded-full"></span>
        <span class="text-xs italic"><?php echo $topbar_subtitle ?? 'Lyceum of Alabang'; ?></span>
    </div>
    ...
    <div class="flex items-center gap-6">
        ...
        <!-- Dark Mode Toggle -->
        <button onclick="toggleDarkMode()" id="theme-toggle" class="p-2.5 bg-slate-50 dark:bg-slate-800 text-slate-500 dark:text-amber-400 rounded-2xl hover:scale-110 active:scale-95 transition-all shadow-sm border border-slate-100 dark:border-slate-700">
            <span id="dm-icon-header" class="text-lg">🌙</span>
        </button>
        ...
        <div class="flex items-center gap-3">
            ...
            <?php echo renderAvatar($_SESSION['user_name'], 'w-10 h-10 border-2 border-white dark:border-slate-800 shadow-lg ring-1 ring-slate-100 dark:ring-slate-700'); ?>
        </div>
    </div>
</header>
